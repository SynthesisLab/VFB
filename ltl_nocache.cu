...
tuple<uint64_t*, int*, int> readJsonFile(const string& filename) {
    ...
    auto fillTrace = [&](const json& trace, int index) {
        ...
        for (int v = 0; v < numVar; ++v) {
            ...
            uint64_t encoded = 0;
            for (int t = 0; t < values.size(); ++t) if (values[t]) encoded |= (1ULL << t);
            inputData[index * numVar + v] = encoded;
        }
        };

    int traceIndex = 0;
    for (const auto& trace : j["positive_traces"]) fillTrace(trace, traceIndex++);
    for (const auto& trace : j["negative_traces"]) fillTrace(trace, traceIndex++);
    ...
    for (int i = 0; i < d_numOfTraces; ++i) {
        ...
        for (int j = 0; j < size; ++j) {
            ...
            if (token >= '0' && token <= '9') {
                ...
                switch (token) {
                case '~': stack[stackIdx] = ~left & (1ULL << d_traceLen[i]) - 1; break;
                case 'X': stack[stackIdx] = left >> 1; break;
                case 'F':
                    left |= left >> 1; left |= left >> 2; left |= left >> 4;
                    left |= left >> 8; left |= left >> 16; left |= left >> 32;
                    stack[stackIdx] = left; break;
                case 'G':
                ...
                case '|': stack[stackIdx] = left | right; break;
                case 'U':
                    right |= left & (right >> 1);  left &= left >> 1;
                    right |= left & (right >> 2);  left &= left >> 2;
                    right |= left & (right >> 4);  left &= left >> 4;
                    right |= left & (right >> 8);  left &= left >> 8;
                    right |= left & (right >> 16); left &= left >> 16;
                    right |= left & (right >> 32);
                    stack[stackIdx] = right; break;
                }
            }

        }

        if (!(stack[0] & 1) && i < d_numOfPosTraces) return false;
        if ((stack[0] & 1) && i >= d_numOfPosTraces) return false;
...
__device__ void numberToFormula(uint64_t n, int size, char formula[maxFormulaSize]) {
    ...
    while (stackIdx >= 0) {
        ...
        if (size == 1) formula[shift] = '0' + n;
            ...
            partSum = 0; opIdx = 0;
            while (n >= partSum + d_numForm[size][opIdx]) partSum += d_numForm[size][opIdx++];

            n -= partSum; size--;
            formula[shift + size] = d_opChar[opIdx];

            // If the operator is unary, push it to the stack
            if (opIdx == 0 || opIdx == 3 || opIdx == 4 || opIdx == 5) stack[++stackIdx] = { n, size, shift };
...
__global__ void processOperator(
    int LTLLen, uint64_t maxTid, uint64_t offset,
    char* d_LTLFormula,
    int* d_foundFlag)
{
    ...
    if (tid < maxTid) {
        ...
        if (found && atomicCAS(d_foundFlag, 0, 1) == 0)
            for (int i = 0; i < LTLLen; ++i) d_LTLFormula[i] = formula[i];
...
uint64_t* generateMatrix(int maxLen) {
    ...
    for (int i = 3; i <= maxLen; ++i) {
        ...
        numForm[i * 8 + 7] = 4 * unaryForm + 2 * commutBinaryForm + notCommutBinaryForm; // Total number of formulas of size i
    }
    ...
    int traceLength)
{
    ...
    checkCuda(cudaMemcpyToSymbol(d_inputData, inputData, numVar * numOfTraces * sizeof(uint64_t)));
    checkCuda(cudaMemcpyToSymbol(d_traceLen, traceLen, numOfTraces * sizeof(int)));
    ...
    for (int i = 0; i < numVar; ++i) {
        ...
        for (int j = 0; j < numOfPosTraces; j++)
            if (!(inputData[j * numVar + i] & 1)) found = false;
        for (int j = numOfPosTraces; j < numOfTraces; j++)
            if (inputData[j * numVar + i] & 1) found = false;
        allLTLs++;
        if (found) return varNames[i];
    }

    // Memory allocation for the potential solution
    char* d_LTLFormula;
    char* LTLFormula = new char[maxFormulaSize]; LTLFormula[0] = '\0';
    ...
    checkCuda(cudaMemcpyToSymbol(d_numForm, numForm, (maxLen + 1) * 8 * sizeof(uint64_t)));
    ...
    for (int LTLLen = 2; LTLLen <= maxLen && !stop; ++LTLLen) {
        ...
        for (int i = 0; i < 7 && !stop; ++i) {
            ...
            if (N > 0) processOperator << <blockSize, 1024 >> > (LTLLen, N, offset, d_LTLFormula, d_foundFlag);
            offset += N; allLTLs += N;

            checkCuda(cudaPeekAtLastError());
            checkCuda(cudaMemcpy(LTLFormula, d_LTLFormula, maxFormulaSize * sizeof(char), cudaMemcpyDeviceToHost));
            if (LTLFormula[0] != '\0') stop = true;
